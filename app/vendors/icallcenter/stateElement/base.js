                        ...
                        if (evtJson.Link) {
                            ...
                            if (this._phone._callId != linkedChannel.Uniqueid) {
                                ...
                                this._phone.callObject = {
                                    ...
                                    beginTime: "",
                                    endTime: "",
                                    province : evtJson.CallerProvince,
                                    city : evtJson.CallerCity,
                                    status: "notDeal"
                                };
                                if (linkedChannel.Data) {
                                    this._phone.callObject.data = linkedChannel.Data;
                                    ...
                                    if(linkedChannel.Data.IVRKEY){
                                        this._phone.callObject.ivrkey = linkedChannel.Data.IVRKEY;
                                    }
                                    for (var key in linkedChannel.Data) {
                                        if (key == "CallSheetID" || key == "callSheetID" || key == "IVRKEY") {
                                            continue;
                                        }
                                        this._phone.callObject[key] = linkedChannel.Data[key];
                                    }
                                }
                                hojo.publish("EvtRing", [this._phone.callObject]);
                            }
    ...
    _setMonitorObjects: function (evtJson) {
        if (evtJson.Event == "ChannelStatus") {
            ...
            if (evtJson.PeerStatus == "Registered")
                isRegistered = true;
            if(evtJson.UserID==this._phone.userId){
                if(evtJson.SipNum != this._phone.sipNo)
                    this._phone.sipNo = evtJson.SipNum;
            }
        ...
        */
        ...
        /* 2.0 phoneBar zxc
        else if (evtJson.Event == "AccountStatus") {// account call status
